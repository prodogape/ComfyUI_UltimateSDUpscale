...
class UltimateSDUpscale:
    ...
    def upscale(self, image, mask, model, positive, negative, vae, upscale_by, seed,
        ...
        if len(mask.shape) == 2:
            mask_pil = tensor_to_pil(mask.unsqueeze(0), 0)
        else:
...
class UltimateSDUpscaleNoUpscale(UltimateSDUpscale):
    ...
    def upscale(self, upscaled_image, mask, model, positive, negative, vae, seed,
                steps, cfg, sampler_name, scheduler, denoise,
                mode_type, tile_width, tile_height, mask_blur, tile_padding,
                seam_fix_mode, seam_fix_denoise, seam_fix_mask_blur,
                seam_fix_width, seam_fix_padding, force_uniform_tiles, tiled_decode):
        upscale_by = 1.0
        return super().upscale(upscaled_image, mask, model, positive, negative, vae, upscale_by, seed,
...
class UltimateSDUpscaleCustomSample(UltimateSDUpscale):
    ...
    def upscale(self, image, mask, model, positive, negative, vae, upscale_by, seed,
                ...
                upscale_model=None,
                custom_sampler=None, custom_sigmas=None):
        return super().upscale(image, mask, model, positive, negative, vae, upscale_by, seed,
                steps, cfg, sampler_name, scheduler, denoise, upscale_model,
                mode_type, tile_width, tile_height, mask_blur, tile_padding,
                seam_fix_mode, seam_fix_denoise, seam_fix_mask_blur,
                seam_fix_width, seam_fix_padding, force_uniform_tiles, tiled_decode,
