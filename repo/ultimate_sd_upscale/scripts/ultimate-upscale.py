...
class USDURedraw():
    ...
    def is_pure_color_patch(self, image, x1, y1, x2, y2, threshold=5):
        # Get the patch
        ...
        first_r, first_g, first_b = pixels[0, 0]
        
        # Check if all pixels are within threshold of the first pixel
        for y in range(height):
            for x in range(width):
                r, g, b = pixels[x, y]
                if (abs(r - first_r) > threshold or 
                    abs(g - first_g) > threshold or 
                    abs(b - first_b) > threshold):
                    return False
        return True
    ...
    def linear_process(self, p, image, rows, cols):
        ...
        for yi in range(rows):
            for xi in range(cols):
                ...
                # Skip if pure color patch
                if self.is_pure_color_patch(image, x1, y1, x2, y2):
                    continue
